<script lang="ts">
    ...
    let {
        ...
        if (filtered_plot_data.length === 0 || !compare_tool_x || !compare_tool_y) {
        ...
        for (const datum of filtered_plot_data) {
            if (datum.times[compare_tool_x] < bounds.min_x) {
                bounds.min_x = datum.times[compare_tool_x];
            }
            if (datum.times[compare_tool_y] < bounds.min_y) {
                bounds.min_y = datum.times[compare_tool_y];
            }

            if (datum.times[compare_tool_x] > bounds.max_x) {
                bounds.max_x = datum.times[compare_tool_x];
            }
            if (datum.times[compare_tool_y] > bounds.max_y) {
                bounds.max_y = datum.times[compare_tool_y];
            }
        }
...
<div class="container flex-row">
    ...
    {#each selected.test_config.tools as tool}
...
<div class="container plot-container" bind:clientWidth={plot_container_width} bind:clientHeight={plot_container_height}>
    {#if compare_tool_x && compare_tool_y && plot_container_width && plot_container_height}
        <Plot 
            axes 
            grid 
        ...
        >
            <Dot data={filtered_plot_data} x={(d) => d.times[compare_tool_x!]} y={(d) => d.times[compare_tool_y!]} stroke={(d) => map_state_to_color(d.state)} />
        </Plot>
    {:else}
        <span>
