<script lang="ts">
	...
	let { compare_total, compare_faster, compare_slower, compare_same, compare_avg_pct } = $derived.by(() => {
		if (!compare_run_x || !compare_run_y || filtered_plot_data.length === 0) {
			return {
				compare_total: 0,
				compare_faster: 0,
				compare_slower: 0,
...
<div class="container flex-row">
	{#each Object.values(PlotDatumState) as state}
		<span
			><div class="color-indicator" style="background-color: {map_state_to_color(state)};"></div>
			{state}</span
		>
	{/each}
</div>
<div class="container plot-container" bind:clientWidth={plot_container_width} bind:clientHeight={plot_container_height}>
	{#if compare_run_x && compare_run_y && plot_container_width && plot_container_height}
		<Plot axes grid x={{ type: 'log', domain: [min_x, max_x] }} y={{ type: 'log', domain: [min_y, max_y] }} width={plot_container_width} height={plot_container_height}>
			...
			<Line data={line_data} x="x" y="y" stroke="#ffffff80" strokeDasharray="10"></Line>
		</Plot>
	{:else}
...
<div class="container">
	{#if compare_run_x && compare_run_y}
		{#if compare_total > 0}
			<div class="compare-overview" style="font-size:0.95rem; line-height:1.3">
				...
				<div>
					...
					<small style="color:#888">(negative = {compare_run_x} faster)</small>
				</div>
			</div>
		{:else}
