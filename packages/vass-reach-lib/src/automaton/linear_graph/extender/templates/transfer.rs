...
//! the decrement guard. A zero fallback is conservative when Z3 cannot provide
//! a usable optimum.
...
pub(super) fn exact_successor_template_bound(
    ...
    let counters = z3_counter_variables(templates[objective_index].coefficients.len());

    // These assertions describe an over-approximation of all valuations that
    // can appear before this edge. The minimized post-update expression is
    // therefore a sound lower bound for the edge target.
    assert_non_negative_counters(&optimizer, &counters);
    assert_enabled_update(&optimizer, update, &counters);
    assert_source_bounds(&optimizer, templates, source_bounds, &counters);

    let objective = successor_objective(&templates[objective_index], update, &counters);
    optimizer.minimize(&objective);

    minimized_objective_value(&optimizer, &objective, cap)
}
...
fn minimized_objective_value(optimizer: &Optimize, objective: &Int, cap: i32) -> i32 {
