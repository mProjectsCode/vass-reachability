...
pub fn same_language<L1: Language, L2: Language<Letter = L1::Letter>>(
    a: &L1,
    b: &L2,
    ...
    for i in 0..max_word_length {
        ...
        for word in combinations {
            let word = word.into_iter().cloned().collect_vec();
            if a.accepts(word.iter()) != b.accepts(word.iter()) {
                println!("{:?}", word);
                return false;
            }
        }
...
pub fn assert_same_language<L1: Language, L2: Language<Letter = L1::Letter>>(
    a: &L1,
    b: &L2,
    max_word_length: usize,
) {
    ...
    for i in 0..max_word_length {
        ...
        for word in combinations {
            let word = word.into_iter().cloned().collect_vec();
            match (a.accepts(word.iter()), b.accepts(word.iter())) {
                (true, false) => {
                    panic!(
...
pub fn assert_inverse_language<L1: Language, L2: Language<Letter = L1::Letter>>(
    a: &L1,
    b: &L2,
    max_word_length: usize,
) {
...
pub fn assert_subset_language<L1: Language, L2: Language<Letter = L1::Letter>>(
    a: &L1,
    b: &L2,
    max_word_length: usize,
) {
